...
function analyzeChunks() {
  ...
  console.log(
    `🎨 CSS: ${formatBytes(totalCSS)} (${((totalCSS / totalAssets) * 100).toFixed(1)}%)`
  )
  console.log(
    `⚡ JavaScript: ${formatBytes(totalJS)} (${((totalJS / totalAssets) * 100).toFixed(1)}%)`
  )
  console.log(
    `🔤 Fonts: ${formatBytes(587.88 * 1024)} (${(((587.88 * 1024) / totalSize) * 100).toFixed(1)}%)`
  )
