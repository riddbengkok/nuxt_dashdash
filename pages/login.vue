        ...
        <div class="card-body">
          ...
          <div v-if="loadingTimeout" class="text-center py-8">
            <div class="alert alert-warning">
              ...
              <div>
                ...
                <div class="text-sm mt-1">Firebase is taking too long to initialize. Please refresh the page.</div>
              </div>
            </div>
          ...
          <div v-if="error" class="alert alert-error mb-4">
            ...
            <div>
              ...
              <div v-if="errorDetails" class="text-sm mt-1">{{ errorDetails }}</div>
            </div>
          </div>
          ...
          <div v-if="showCorsWarning" class="alert alert-warning mb-4">
            ...
            <div>
              ...
              <div class="text-sm mt-1">
                Please allow popups and cookies for this site to enable Google sign-in.
              </div>
            </div>
          </div>

          <!-- Social Login Buttons -->
          <div v-if="!loading && !loadingTimeout" class="mb-6 grid justify-center">
            <button 
              @click="handleGoogleSignIn"
              :disabled="isSigningIn"
              class="btn w-full max-w-sm bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 shadow-sm disabled:opacity-50"
            >
              <font-awesome-icon :icon="['fab', 'google']" class="w-5 h-5 mr-2" />
...
onMounted(() => {
  setTimeout(() => {
    if (loading.value) {
      ...
      console.error('Firebase loading timeout - environment variables may not be loaded')
    }
...
const handleGoogleSignIn = async () => {
  try {
    ...
    if (err.message.includes('popup was blocked')) {
      ...
      errorDetails.value = 'This domain is not configured for authentication. Please contact support.'
    } else if (err.message.includes('cancelled')) {
      error.value = 'Sign-in Cancelled'
      errorDetails.value = 'You cancelled the sign-in process.'
    } else {
      error.value = 'Sign-in Failed'
      errorDetails.value = err.message || 'An unexpected error occurred. Please try again.'
    }
  } finally {
...
watch(user, (newUser) => {
  console.log('Login: User state changed:', newUser)
  if (newUser) {
...
onMounted(() => {
  ...
  if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
    ...
    errorDetails.value = 'This site requires HTTPS for authentication to work properly.'
  }
})
</script>
