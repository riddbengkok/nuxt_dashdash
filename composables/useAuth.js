export const useAuth = () => {
  ...
  const tokenExpiryThreshold = config.public.tokenExpiryThreshold || 5 * 60 * 1000 // 5 minutes threshold
  ...
  const checkTokenExpiration = async () => {
    if ($firebase && $firebase.auth.currentUser) {
      try {
        ...
        if (timeUntilExpiry <= tokenExpiryWarning && timeUntilExpiry > tokenExpiryThreshold) {
          console.warn(`Token will expire in ${Math.floor(timeUntilExpiry / 1000 / 60)} minutes`)
        }
  ...
  onMounted(() => {
    ...
    if ($firebase) {
      ...
      const unsubscribe = $firebase.onAuthStateChanged(async (authUser) => {
